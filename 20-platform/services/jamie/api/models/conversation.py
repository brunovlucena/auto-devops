...
if __name__ == "__main__":
    ...
    manager.add_message("test_session", "user123", "How's my cluster?", True)
    manager.add_message("test_session", "user123", "Your cluster's running like a dream, mate!", False)
    manager.add_message("test_session", "user123", "Any alerts firing?", True)
    
    # Get context
    context = manager.get_conversation_context("test_session")
    print("Conversation Context:", json.dumps(context, indent=2))
    
    # Get summary
    summary = manager.get_conversation_summary("test_session")
